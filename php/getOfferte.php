<?php
   ...
   $resultaat = get($query);
    ...
    $query = "SELECT Clientnr, Company, OfferteNaam, OfferteOmschrijving FROM offertes inner join klanten on offertes.KlantID = klanten.Clientnr where offertes.OfferteID = " . $offerteID;
    $resultaat2 = get($query);

    $outp2 = "";
    while($rs = $resultaat2->fetch_array(MYSQLI_ASSOC)) {
        if ($outp2 != "") {$outp2 .= ",";}
        $outp2 .= '{"klantID":"' . $rs["Clientnr"] . '",';
